...
-   **MBTI 数据来源**：题目与人格特点来自 `素材/MBTI题库和答案/` 下 CSV，**不使用 JSON**，可用 Navicat 直接导入；详见 [CSV 与表结构说明](./CSV与表结构说明.md)。
...
| strengths_test_dimensions        | MBTI 四个维度说明 ← 03-答案-MBTI 的四个维度.csv                                                  |
| strengths_test_dimension_sides   | 四个维度八个面说明 ← 03-答案-四个维度八个面.csv                                                  |
| strengths_test_answer            | 测试答案（MBTI 十六种性格类型等测试结果的解释内容）← 03-答案-十六种性格类型.csv                  |
...
### 4.6 strengths_test_dimensions（MBTI 四个维度）← 03-答案-MBTI 的四个维度.csv
...
**唯一**：`(test_type, result_code)`。**索引**：`test_type`、`result_code`。存储测试结果的解释内容（MBTI 十六种性格类型等），无 JSON，Navicat 可从 CSV 直接导入。
...
| e_score          | int(11)             | 非空，默认 0 | 外向(E)得分                                                 |
| i_score          | int(11)             | 非空，默认 0 | 内向(I)得分                                                 |
| s_score          | int(11)             | 非空，默认 0 | 实感(S)得分                                                 |
| n_score          | int(11)             | 非空，默认 0 | 直觉(N)得分                                                 |
| t_score          | int(11)             | 非空，默认 0 | 思考(T)得分                                                 |
| f_score          | int(11)             | 非空，默认 0 | 情感(F)得分                                                 |
| j_score          | int(11)             | 非空，默认 0 | 判断(J)得分                                                 |
| p_score          | int(11)             | 非空，默认 0 | 知觉(P)得分                                                 |
...
**八面得分说明**：提交答案时，后端按 dimension_side 汇总 E/I/S/N/T/F/J/P 各端得分并写入上述 8 个字段；报告接口据此计算百分比返回 dimension_scores（如 E/(E+I)×100、I/(E+I)×100）。
...
**开发实现建议**：在计分逻辑中，对每个维度（E-I、S-N、T-F、J-P，与 strengths_test_dimensions.dimension_code 一致）先汇总 E/I、S/N、T/F、J/P 得分；若两端分数相等，则按上表取右侧一列（I、N、F、P）；否则取分数高的一端。最后将四个字母组合成 result_code（如 INTJ）。
...
5. **组合 result_code**：按维度顺序（E-I→ 第 1 字母，S-N→ 第 2 字母，T-F→ 第 3 字母，J-P→ 第 4 字母）拼成 **result_code**，如 INTJ。
6. **写入 strengths_test_results_records**：`test_type=MBTI`，`result_code=INTJ`，同时写入 `e_score`、`i_score`、`s_score`、`n_score`、`t_score`、`f_score`、`j_score`、`p_score` 八个面得分；同一 openid/session + test_type 若已有未付费记录可更新该条，否则新增；`is_paid=0`。
...
-   **03-答案-MBTI 的四个维度.csv** → `strengths_test_dimensions`（需补 test_type=MBTI；维度方面 →dimension_aspect，维度特质对 →dimension_trait_pair，维度代码 →dimension_code）。
