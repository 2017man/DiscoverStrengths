...
- 前端：支付页调用 `POST /order/create`，获取 `pay_url`、`out_trade_no`（可选 `epay_order_id`）后：
  - **跳转支付**：`window.location.href = pay_url` 跳转易支付；支付完成后易支付若配置了同步跳转（`EPAY_RETURN_URL`），会跳回前端报告页，报告页调 `GET /mbti/report` 或 `GET /order/status` 即可拿到最新 `is_paid`。
  - **展示二维码**（易支付 isHtml=0）：在当前页展示 `pay_url` 二维码，保存 `out_trade_no`，定时轮询 `GET /order/check-payment?out_trade_no=xxx`，当返回 `data.paid === true` 时提示支付成功并跳转报告页。
- 测试免支付：后端返回 `skip_payment: true` 时，前端直接跳转报告页。
- 支付成功：易支付异步回调后端，后端更新订单与 `is_paid`；同步跳转由 `EPAY_RETURN_URL` 配置。
- **前端是否需要改**：若当前仅「跳转 pay_url」且易支付配置了同步跳转，报告页已调 mbti/report 或 order/status 则**无需改**。若需支持「二维码展示 + 轮询」，需对接 `GET /order/check-payment?out_trade_no=xxx` 并保存创建订单返回的 `out_trade_no`。详见《接口文档》5.4。
