...
// POST（application/x-www-form-urlencoded 默认）
this.$http.sendRequest('mbti/submit', 'POST', {
  answers: [1, 0, 1, ...],  // 选项索引或分值
  result_id: 123
}).then(res => {
  if (res.data.code === 200) {
    // 跳转结果页等
...
// 创建订单：传 result_id，后端从 strengths_test_results_records 取 test_type 与金额
...
- **匿名答题**：可不登录；用 `uni.setStorageSync('mbti_answers', [...])` 暂存选项，提交时一并上传。
...
- **题目列表**：调用「MBTI 题目列表」，按 `question_number` 排序展示；选项用 `options` JSON。
- **提交答案**：调用「MBTI 提交答案」，传题目答案；返回 `result_id`、`result_code`（如 INTJ）、预览内容等。
- **报告是否付费**：调用「报告详情」或「订单状态」，若已付费则展示完整报告，否则展示预览 + 支付按钮。
- **支付**：调用「创建订单」获取易支付参数，跳转支付；支付成功后由后端回调，前端轮询或跳转「报告页」并带 `result_id`。
